...
exports.handler = async (event, context) => {
  console.log('Download redirect function called:', event.httpMethod);
  ...
  try {
    ...
    console.log('Fetching latest release for redirect download');
...
<body>
  ...
  <p>Available files: ${releaseData.assets.map(a => a.name).join(', ')}</p>
    ...
    if (!downloadUrl) {
      ...
      throw new Error('Could not get download URL from GitHub');
    }
    ...
    console.error('Error getting download redirect:', error);
...
<body>
  ...
  <p>Failed to get download URL: ${error.message}</p>
